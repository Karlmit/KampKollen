...
function SortableOptionRow({ option, onUpdate, onDelete, onImageUpload }: {
  ...
  return (
    <div ref={setNodeRef} style={{ ...style, display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 10px',
      ...
      />
      <label style={{ cursor: 'pointer', fontSize: '16px', color: 'var(--text-muted)', flexShrink: 0 }} title="Upload image">
        🖼
        <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { if (e.target.files?.[0]) onImageUpload(e.target.files[0]) }} />
      </label>
      <button type="button" onClick={onDelete} style={{ color: 'var(--accent-warm)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', flexShrink: 0 }}>×</button>
    </div>
  )
}
...
export function QuizEditorPage() {
  ...
  if (isLoading || !stateData) return <Layout title="Edit Quiz" back={backUrl}><LoadingSpinner /></Layout>

  const questions: any[] = data?.questions ?? []
  const challengeName = data?.challenge?.name ?? 'Quiz'

  return (
    <Layout title={`Edit: ${challengeName}`} back={backUrl}>
      <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '16px' }}>
        {questions.length} question{questions.length !== 1 ? 's' : ''}. Tap ○ on an option to mark it as the correct answer.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {questions.map((q: any, qi: number) => (
          <Card key={q.id}>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', flexShrink: 0, alignItems: 'flex-start' }}>
                ...
                {qi < questions.length - 1 && (
                  <button type="button" onClick={() => { const ids = questions.map((x: any) => x.id); [ids[qi], ids[qi + 1]] = [ids[qi + 1], ids[qi]]; reorder.mutate(ids) }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '14px', color: 'var(--text-muted)' }}>↓</button>
                )}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', marginBottom: '10px', fontSize: '13px', color: 'var(--text-muted)' }}>
            ...
            {q.isFreeText ? (
              <div style={{ padding: '10px 12px', borderRadius: 'var(--radius-sm)', background: 'var(--surface)', fontSize: '13px', color: 'var(--text-muted)', fontStyle: 'italic' }}>
                {t('quiz.freeTextQuestion')} — {t('quiz.freeTextPlaceholder')}
              </div>
            ) : (
              <>
                <DndContext
